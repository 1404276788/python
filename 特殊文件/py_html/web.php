...
for ($now+=1;$now<=$total;$now++) { 
	...
	if (!$re && file_exists($filePath)) {
		...
		$furl = $_SERVER["REQUEST_SCHEME"]."://".$_SERVER["SERVER_NAME"].$dirname.$filePath."\r\n";
		file_put_contents("./text.txt", $furl,FILE_APPEND);
		file_put_contents("./count_config.php", "<?php\r\n ".'$total'."={$total};//总条数\r\n ".'$now'."={$now};//当前请求条数\r\n");

		// gk add 2019.6.21
		$gki = 1;
		if (mt_rand(1,6) == 3 && $gki < 21 && $wl_status ==1){
			file_put_contents("./text20.txt", $furl,FILE_APPEND);
			$gki = $gki + 1;
		}
	}
