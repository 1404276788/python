...
#C:\Users\admin\Desktop\snsnb_err_id.txt
...
for line in open("C:/Users/admin/Desktop/snsnb_err_id1.txt"):
    aid=line.strip('\n')
    # print(aid)
    for dbname in dbnamelist:
        for dbtable in dbtablelist:
            sql='select aid,catid from {}.ivf_{} where aid={}'.format(dbname,dbtable,aid)
            res=db_rand_wl(dbname,sql)
            f=open('snsnb\\err.txt','a',encoding='utf-8')
            for list_m in res:
                s_aid=str(list_m[0])
                s_catid=str(list_m[1])
                # print("aid:"+s_aid+","+"catid:"+s_catid)
                f.write("aid:"+s_aid+","+"catid:"+s_catid+'\n') 
            f.close()
